$execute as @p[scores={permPNo=$(s)}] at @s run summon tnt ~ ~1 ~ {fuse:400,explosion_power:32,Tags:["ent"]}
$execute as @p[scores={permPNo=$(s)}] at @s run summon tnt ~ ~1 ~ {fuse:405,explosion_power:32,Tags:["ent"]}

$title @p[scores={permPNo=$(s)}] title [{"text":">> ","color":"gold","bold":true},{"text":"NUKE!!!","color":"red"},{"text":" <<","color":"gold"}]

$execute if score Lang Core matches 1 run title @p[scores={permPNo=$(s)}] subtitle [{"text":"Explodes in ","color":"red","bold":true},{"text":"20","color":"green"},{"text":" secs -- RUN TF AWAY!!","color":"red"}]

$execute if score Lang Core matches 2 run title @p[scores={permPNo=$(s)}] subtitle [{"text":"20","color":"green"},{"text":"秒后爆炸 -- 赶紧逃命!!","color":"red"}]
