...
execute if score Lang Core matches 1 run tellraw @a [{"text":"Map created by Jerries","color":"yellow","bold":true},{"text":" (Map version 1.0)","color":"yellow","bold":false,"italic":true}]
execute if score Lang Core matches 2 run tellraw @a [{"text":"地图由 Jerries 制作","color":"yellow","bold":true},{"text":" (地图版本 1.0)","color":"yellow","bold":false,"italic":true}]
